...
function widget_video_control($widget_args) 
{
	...
	if ( !$updated && !empty($_POST['sidebar']) ) {
		...
		foreach ( (array) $_POST['widget-video'] as $widget_number => $widget_content ) {
			...
			$options[$widget_number] = compact( 'title', 'source', 'id', 'width', 'height');
		}
...
function widget_video_register() {
	...
	foreach ( array_keys($options) as $o ) {
		...
		if ( !isset($options[$o]['title']) || !isset($options[$o]['source'])  || !isset($options[$o]['id'])  || !isset($options[$o]['width'])  || !isset($options[$o]['height']) )
			continue;
